...
from coda_qubic.qasm import QASMConversionError, ir_to_openqasm, openqasm_to_ir
...
# ===================================================================
# 8. OpenQASM round-trip
# ===================================================================


class TestOpenQASMRoundTrip:
    """Verify lossless OpenQASM ↔ NativeGateIR conversion."""

    # -- superconducting_cz: QASM → IR → QASM -------------------------

    def test_cz_qasm_roundtrip(self) -> None:
        qasm = (
            "OPENQASM 3.0;\n"
    ...
    def test_cz_single_gate_roundtrips(self) -> None:
        for gate_line, target_gate in [
            ("rx(0.123) q[0];", "rx"),
            ("ry(-0.456) q[1];", "ry"),
            ...
            ir = openqasm_to_ir(qasm, target="superconducting_cz")
            assert ir.gates[0].gate.value == target_gate
            roundtripped = ir_to_openqasm(ir)
            ir2 = openqasm_to_ir(
                roundtripped,
                target="superconducting_cz",
    ...
    def test_cnot_qasm_roundtrip(self) -> None:
        qasm = (
            "OPENQASM 3.0;\n"
            'include "stdgates.inc";\n'
            "qubit[4] q;\n"
    ...
    def test_cnot_gate_mapping(self) -> None:
        qasm = (
            "OPENQASM 3.0;\n"
            'include "stdgates.inc";\n'
            "qubit[2] q;\n"
    ...
    # -- IR → QASM → IR for superconducting_cz ------------------------

    def test_cz_ir_roundtrip(self) -> None:
        ir = NativeGateIR(
            target="superconducting_cz",
            num_qubits=3,
            gates=[
    ...
    # -- IR → QASM → IR for superconducting_cnot ----------------------

    def test_cnot_ir_roundtrip(self) -> None:
        ir = NativeGateIR(
            target="superconducting_cnot",
            num_qubits=4,
            gates=[
    ...
    # -- QASM text stability: QASM → IR → QASM produces same text -----

    def test_cz_qasm_text_stability(self) -> None:
        qasm = (
            "OPENQASM 3.0;\n"
            'include "stdgates.inc";\n'
            "qubit[2] q;\n"
    ...
    def test_cnot_qasm_text_stability(self) -> None:
        qasm = (
            "OPENQASM 3.0;\n"
            'include "stdgates.inc";\n'
            "qubit[2] q;\n"
